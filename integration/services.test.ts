  ...
  it("connects to PostgreSQL and Redis after migrations", async () => {
    ...
    const tables = await sql`
      ...
      WHERE table_schema = 'public'
        AND table_name IN ('app_settings', 'evaluators', 'scenarios', 'test_runs', 'model_results', 'model_result_turns', 'evaluations', 'evaluation_history', 'model_artifacts', 'execution_environments', 'experiments', 'experiment_arms')
    `;
