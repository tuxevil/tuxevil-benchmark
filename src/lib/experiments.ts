...
function summarizePair(
  ...
  for (const [caseId, base] of baseline) {
    ...
    if (!next) continue;
    totalPairs += 1;
    if (base.value !== next.value) changed += 1;
    ...
    if (typeof base.success === "boolean" && typeof next.success === "boolean") {
      ...
      if (base.success && !next.success) lostSuccesses += 1;
      if (!base.success && next.success) gainedSuccesses += 1;
    }
  }
  ...
  return {
    ...
    agreementRate: totalPairs > 0 ? 1 - churnRate : 1,
    lostSuccesses,
    gainedSuccesses,
...
export function comparePairedObservations(
  ...
  if (options.baselineRepeat) {
    ...
    validity = intrinsicChurnRate <= threshold ? "VALID" : "INVALID";
